...
export default function DescriptionForm({ id, description }: Props) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    await errorHandler(
      async () => {
        await req.updateCourse(id, { description: data.description })
        ...
        <div className="mt-4 text-sm">
          {isEditing ? (
            <textarea
              ...
              rows={3}
              className="flex w-full resize-none items-center justify-center rounded-md border border-[#2c2c2c] bg-transparent px-3 py-1.5 outline-none"
            />
          ) : (
